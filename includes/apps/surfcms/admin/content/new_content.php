...
*/
  ...
  if ( !in_array($admin['id'], explode(',',$ocInfo->surfcms_content_administrators)) )  {
    ...
    // build width input
    $checked = ( $ocInfo->surfcms_content_width==='' ? 0 : (int)$ocInfo->surfcms_content_width);
    ...
    for ($i=1; $i<=24; $i++) {
      $width_fields .= tep_draw_radio_field('surfcms_content_width', (string)$i, ($i == $checked ? true : false)) . '&nbsp;<span style="font-size: 1.3em;">' . (string)$i . '</span>&nbsp;&nbsp;&nbsp;' . ($i==12 ? '<br />' . tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' : '');
    }
